...
(() => {
  try {
    if (ENCRYPTION_KEY && Buffer.from(ENCRYPTION_KEY, 'utf8').length === 32) {
      ...
      logger.warn('[CRYPTO] ENCRYPTION_KEY manquante ou invalide (32 chars requis). Le chiffrement est indisponible jusqu\'à configuration.');
      key = null;
    }
  } catch (e) {
    logger.warn('[CRYPTO] Impossible d\'initialiser la clé de chiffrement:', (e as Error)?.message);
    key = null;
  }
})();
...
export function encrypt(text: string): string {
  if (!key) {
    ...
    logger.warn('[CRYPTO] encrypt() appelé sans clé valide en environnement non-production. Retour en clair.');
    return text;
  }
...
export function decrypt(text: string): string {
  try {
    ...
    if (!key) {
      logger.warn('[CRYPTO] decrypt() appelé sans clé valide. Retour du texte original.');
      return text;
    }
