...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchMe = useCallback(async () => {
    ...
    const fetchModules = useCallback(async () => {
      ...
      try {
        ...
        if (currentOrganization?.id && currentOrganization.id !== 'all') {
          ...
          console.log('[AuthProvider] 🌐 Vue globale - Aucune organisation spécifique sélectionnée');
        ...
        // 🎯 FILTRER : Garder seulement les modules ACTIFS pour cette organisation
        const activeModules = rawModules.filter((module: ModuleAccess) => module.isActiveForOrg === true);
        ...
        const mappedModules = activeModules.map((module: ModuleAccess) => ({
          ...module,
          // Mapper isActiveForOrg vers isActiveInOrg pour la compatibilité TypeScript
          isActiveInOrg: module.isActiveForOrg !== undefined ? module.isActiveForOrg : module.isActiveInOrg,
          // S'assurer que la feature est correctement mappée
    ...
    }, [user, currentOrganization]); // 🔧 DÉPENDANCES STABLES POUR ÉVITER LES BOUCLES
