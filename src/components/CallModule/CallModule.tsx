/**
 * 📞 CallModule - Module d'appel révolutionnaire avec IA intégrée
 * 
 * Architecture modulaire complète :
 * - 🎯 Orchestration complète de l'appel avec statuts standardisés
 * - 🤖 Assistant IA conversationnel intégré
 * - 📅 Calendrier Google intelligent
 * - 🎙️ Transcription vocale temps réel
 * - 📊 Analytics d'appel en direct
 * - 🎯 Affichage automatique du Smart Agenda quand le statut est "meeting_scheduled"
 */
...
/**
 * 📞 CallModule - Module d'appel central avec IA intégrée
 */
export default function CallModule({ leadId: propLeadId, onClose }: CallModuleProps): React.ReactElement {
  const { leadId: urlLeadId } = useParams<{ leadId: string }>();
  const finalLeadId = propLeadId || urlLeadId!;

  // 🎣 Hooks centralisés
  const { lead, loading: leadLoading, error: leadError } = useLead(finalLeadId);
  const callLogic = useCallLogic(finalLeadId);
  const aiAssistant = useAIAssistant(finalLeadId, lead);
  const googleCalendar = useGoogleCalendar(finalLeadId, lead);
