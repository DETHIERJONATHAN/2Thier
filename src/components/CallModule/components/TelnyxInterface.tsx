...
export const TelnyxInterface: React.FC<TelnyxInterfaceProps> = ({
  ...
  const uiState = useMemo(() => {
    // Priorité : champ root phone puis lead.data.phone
    const phoneNumber = lead.phone || lead.data?.phone;
    const canCall = !callState.isInProgress && !isLoading && phoneNumber;
    ...
    return {
      ...
      phoneDisplay: phoneNumber ? 
        phoneNumber.replace(/(\+\d{2})(\d{1})(\d{8})/, '$1 $2 $3') :
        'Aucun numéro'
    };
  }, [callState, isLoading, lead.phone, lead.data?.phone, lead.id]);
      ...
      <div style={{ textAlign: 'center', marginBottom: 24 }}>
        <Title level={4} style={{ margin: 0, color: '#1890ff' }}>
          {lead.firstName && lead.lastName ? `${lead.firstName} ${lead.lastName}` : 
           (lead.firstName || lead.lastName || lead.data?.name || lead.name || `Lead ${lead.id?.slice(0,8)}`)}
        </Title>
        ...
        {!uiState.canCall && !callState.isInProgress && (
          <div style={{ textAlign: 'center', color: '#faad14' }}>
            <Text type="warning" style={{ fontSize: 11 }}>
              {!uiState.phoneNumber ? 'Numéro manquant' : 'Non disponible'}
            </Text>
          </div>
        )}
      </div>
