...
export const LeadInfoPanel: React.FC<LeadInfoPanelProps> = ({
  ...
  const leadInfo = useMemo(() => {
    ...
    // 🎯 Priorité : champs racines puis lead.data
    const address = formatAddress(lead.data?.address ?? lead.data?.city);
    
    // Composition du nom : priorité aux champs racines
    ...
    return {
      name: displayName,
      email: lead.email || lead.data?.email || 'Email non renseigné',
      phone: lead.phone || lead.data?.phone || 'Téléphone non renseigné',
      company: lead.company || lead.data?.company || 'Entreprise non renseignée',
