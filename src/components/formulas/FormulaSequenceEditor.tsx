...
const FormulaSequenceEditor: React.FC<FormulaSequenceEditorProps> = ({ formula }) => {
    ...
    const sequence = React.useMemo(() => formula.sequence || [], [formula.sequence]);
    ...
    // (Déplacé plus bas après la déclaration de updateFormulaSequence pour éviter une erreur de portée)
    ...
    const updateFormulaSequence = useCallback(async (newSequence: FormulaItem[]) => {
        try {
            ...
            // updated locally inline (optimistic)
            
            // Mettre à jour l'état local du composant parent (hack React pour forcer la mise à jour)
            // (suppression variables inutilisées) mise à jour optimiste directe
            ...
            if (draggedItem) {
                ...
                // Créer un nouvel item avec un ID unique pour éviter les duplications
                                ...
                                if (draggedItem.type === 'cond') {
                                    ...
                                    newItem = {
                                        id: `switch-${Date.now()}-${Math.random().toString(36).slice(2,9)}`,
                                        type: 'switch',
                                        value: 'switch',
                                        label: draggedItem.label || 'Switch',
                                        switchFieldId: draggedItem.switchFieldId,
                                        switchPart: draggedItem.switchPart || 'selection',
                                        cases: [],
                                        defaultSeq: []
                                    } as unknown as FormulaItem;
                                } else {
    ...
    }, [sequence, updateFormulaSequence, formula, formulaId]);
    ...
    // Ecoute les insertions via la mini barre d'outils (placé ici après updateFormulaSequence pour éviter la TDZ)
    useEffect(() => {
        const handler = (e: Event) => {
            const detail = (e as CustomEvent).detail as { type: string; value?: string; label?: string; formulaId?: string };
            if (!detail || detail.formulaId !== formulaId) return;
            let newItem: FormulaItem | null = null;
            if (detail.type === 'operator') {
                newItem = { type: 'operator', value: String(detail.value || '+'), label: detail.label || String(detail.value || '+'), id: `operator-${Date.now()}` } as unknown as FormulaItem;
            } else if (detail.type === 'value') {
                newItem = { type: 'value', value: String(detail.value ?? '0'), label: detail.label || String(detail.value ?? '0'), id: `value-${Date.now()}` } as unknown as FormulaItem;
            }
            if (newItem) {
                const newSequence = [...sequence, newItem];
                formula.sequence = newSequence;
        ...
        document.addEventListener('formula-toolbar-insert', handler);
        const clearHandler = (e: Event) => {
            const detail = (e as CustomEvent).detail as { formulaId?: string };
            if (!detail || detail.formulaId !== formulaId) return;
            const newSequence: FormulaItem[] = [];
            formula.sequence = newSequence;
            setForceUpdateKey(prev => prev + 1);
            updateFormulaSequence(newSequence);
        };
        document.addEventListener('formula-toolbar-clear', clearHandler);
        return () => {
            document.removeEventListener('formula-toolbar-insert', handler);
            document.removeEventListener('formula-toolbar-clear', clearHandler);
        };
    }, [sequence, updateFormulaSequence, formula, formulaId]);
    ...
    <DndContext
        ...
        >
            <div 
                ref={setNodeRef} 
                className={`min-h-[140px] p-4 border-2 border-dashed rounded-lg formula-drop-zone 
                    ${isOver ? 'border-blue-400 bg-blue-50 ring-2 ring-blue-200/60' : 'border-gray-300 hover:border-blue-300 hover:bg-blue-50/40'} 
                ...
                onDrop={(e) => {
                    ...
                    if (type) {
                        ...
                        if (type === 'cond') {
                            ...
                            const sfid = e.dataTransfer.getData('switch-field-id');
                            const flabel = e.dataTransfer.getData('switch-field-label');
                            newItem = {
                                id: `switch-${Date.now()}-${Math.random().toString(36).slice(2,9)}`,
                                type: 'switch',
                                value: 'switch',
                                label: flabel ? `Switch ${flabel}` : 'Switch (déposer un champ)',
                                switchFieldId: sfid || undefined,
                            ...
                            newItem = {
                                ...
                                refFormulaId: refId,
                                value: refId,
                                label: refLabel || refId,
                                id: `formula_ref-${refId}-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`
                            } as unknown as FormulaItem; // cast pour compat billet
                        } else if (type === 'adv_part') {
                            ...
                            newItem = {
                                type: 'adv_part',
                                fieldId,
                                part: part as 'selection' | 'extra' | 'nodeId',
                                value: `${fieldId}.${part}`,
                                label: label || `${fieldId}.${part}`,
                    ...
                    <div className="flex-1">
                        {sequence.length === 0 ? (
                                    <div className="empty-state">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="empty-icon">
                                            <path d="M14 3v4a1 1 0 0 0 1 1h4" />
                                            <path d="M18 15v4a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h8l6 6v6z" />
                                            <path d="M12 15V9" />
                                            <path d="m9 12 3-3 3 3" />
                                        </svg>
                                        <p className="empty-title">Glissez-déposez ou cliquez sur un champ à gauche…</p>
                                    </div>
                        ) : (
                                        ...
                                        return (
                                            <SortableFormulaItem
                                                ...
                                                onUpdate={(nextItem) => {
                                                    ...
                                                    newSeq[index] = nextItem as unknown as FormulaItem;
                                                    formula.sequence = newSeq;
                                                    setForceUpdateKey(prev => prev + 1);
                                                    updateFormulaSequence(newSeq);
                                                }}
                                            />
                                        );
                                    })}
