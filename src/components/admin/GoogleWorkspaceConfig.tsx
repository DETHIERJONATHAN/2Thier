...
import { GoogleOutlined, CheckCircleOutlined, ExclamationCircleOutlined, LoginOutlined, EyeInvisibleOutlined, EyeTwoTone, RedoOutlined, CopyOutlined, SettingOutlined, CloudOutlined } from '@ant-design/icons';
...
const GoogleWorkspaceConfig: React.FC<GoogleWorkspaceConfigProps> = ({ organizationId }) => {
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      const response = await api.get(`/organizations/${organizationId}/google-workspace/config`);
  ...
  const checkDomainAndLoadConfig = useCallback(async () => {
    ...
    try {
      ...
      if (orgResponse.success && orgResponse.data) {
        ...
        const statusResponse = await api.get(`/organizations/${organizationId}/google-workspace/domain-status`);
  ...
  const checkGoogleConnection = useCallback(async () => {
    try {
      ...
      const response = await api.get(`/google-auth/status?organizationId=${organizationId}`);
  ...
  const handleSave = async (values: GoogleWorkspaceConfig) => {
    ...
    try {
      ...
      const response = await api.post(`/organizations/${organizationId}/google-workspace/config`, values);
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      const response = await api.get(`/google-auth/connect?organizationId=${organizationId}`);
