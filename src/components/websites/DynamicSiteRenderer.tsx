  ...
  return (
    <header style={{ ...style, padding: '20px 50px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        {/* Logo */}
        {content.logo && (
          <img loading="lazy" src={content.logo} alt="Logo" style={{ height: '50px' }} />
        )}
    ...
    <section style={{ ...style, padding: '80px 50px' }}>
      ...
      <div style={{ 
        ...
        {content.columns?.map((col: unknown, index: number) => (
          <div key={`item-${index}`} style={{ textAlign: 'center' }}>
            {col.image && (
              <img loading="lazy" src={col.image} 
                alt={col.title} 
                style={{ 
                  width: '100%', 
                  maxWidth: '200px',
  ...
  return (
    <footer style={{ ...style, padding: '60px 50px 30px', borderTop: '1px solid #e5e5e5' }}>
      <div style={{ 
        ...
        {content.logo && (
          <div>
            <img loading="lazy" src={content.logo} alt="Logo" style={{ height: '40px', marginBottom: '15px' }} />
            {content.description && (
