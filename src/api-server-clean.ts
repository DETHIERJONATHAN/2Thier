...
// Sentry doit être initialisé AVANT tout autre import pour intercepter les erreurs
...
// Permissions-Policy header (camera/microphone needed for video calls)
...
// 🛡️ #8 CSRF Protection — require custom header on mutating requests
// Browsers won't send custom headers in cross-origin requests without preflight,
// so this prevents CSRF from simple form submissions.
app.use((req, res, next) => {
  const safeMethods = ['GET', 'HEAD', 'OPTIONS'];
  if (safeMethods.includes(req.method)) return next();
  // Allow requests with Authorization header (API tokens) or custom CSRF header
  if (req.headers['authorization'] || req.headers['x-csrf-protection'] || req.headers['x-requested-with']) {
    return next();
  }
  // Allow file uploads (multipart) which always come from our frontend (CORS validated)
  if (req.headers['content-type']?.includes('multipart/form-data')) {
    return next();
  }
  logger.warn(`🚫 [CSRF] Requête ${req.method} ${req.path} bloquée — header CSRF manquant`);
  return res.status(403).json({ message: 'CSRF protection: custom header required' });
});

// #32 Cache-Control headers for API responses
app.use('/api', (req, res, next) => {
  if (req.method === 'GET') {
    // Short cache for GET API requests, must revalidate
    res.set('Cache-Control', 'private, no-cache, must-revalidate');
    // ETag is generated by Express automatically when response is set
  } else {
...
if (!process.env.SESSION_SECRET && process.env.NODE_ENV === 'production') {
  logger.error('❌ FATAL: SESSION_SECRET non défini en production — arrêt immédiat');
  process.exit(1);
}
const sessionSecret = process.env.SESSION_SECRET || 'crm-dev-session-only';
...
if (process.env.NODE_ENV !== 'production') {
  import('./lib/swagger').then(({ generateSwaggerSpec }) => {
    import('swagger-ui-express').then((swaggerUi) => {
      ...
      logger.debug('📖 [SWAGGER] API docs available at /api/docs');
    });
  }).catch((err) => {
    logger.warn('📖 [SWAGGER] Failed to load swagger:', (err as Error)?.message);
  });
}
...
// Sentry error handler (doit être avant notre handler custom)
if (process.env.SENTRY_DSN) {
    ...
    const server = httpServer.listen(port, '0.0.0.0', () => {
      ...
      setInterval(async () => {
        try {
          ...
          // #43 Delete expired Stories (keep highlights)
          const deletedStories = await db.story.deleteMany({
            where: { expiresAt: { lt: now }, isHighlight: false },
          });

          // Delete expired sessions from connect-pg-simple store
          try {
            await db.$executeRawUnsafe(`DELETE FROM "session" WHERE "expire" < NOW() - INTERVAL '1 day'`);
          } catch { /* session table may not exist */ }

          if (expiredCount > 0 || deletedPins.count > 0 || deletedStories.count > 0) {
            logger.debug(`🕯️ [WAX-CLEANUP] ${expiredCount} messages expired, ${deletedPins.count} pins deleted, ${deletedStories.count} stories pruned`);
          }
        } catch (err) {
