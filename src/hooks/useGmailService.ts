...
export const useGmailService = () => {
  ...
  const getMessages = useCallback(async (params: {
    ...
    return handleApiCall(async () => {
      ...
      console.log('[useGmailService] 📡 Making API call to /gmail/messages...');
      const response = await apiRef.current.get('/gmail/messages', { params: serverParams });
  ...
  const getMessage = useCallback(async (messageId: string, format: string = 'full') => {
    return handleApiCall(async () => {
      const response = await apiRef.current.get(`/gmail/messages/${messageId}`, {
        params: { format }
      });
      return response;
    });
  ...
  const modifyMessage = useCallback(async (messageId: string, addLabelIds: string[] = [], removeLabelIds: string[] = []) => {
    return handleApiCall(() => 
      apiRef.current.post(`/gmail/messages/${messageId}/modify`, { addLabelIds, removeLabelIds })
    );
  }, [handleApiCall]);

  const trashMessage = useCallback(async (messageId: string) => {
    return handleApiCall(() => apiRef.current.post(`/gmail/messages/${messageId}/trash`));
  }, [handleApiCall]);

  const untrashMessage = useCallback(async (messageId: string) => {
    return handleApiCall(() => apiRef.current.post(`/gmail/messages/${messageId}/untrash`));
  }, [handleApiCall]);

  const deleteMessage = useCallback(async (messageId: string) => {
    return handleApiCall(() => apiRef.current.delete(`/gmail/messages/${messageId}`));
  }, [handleApiCall]);

  const emptyTrash = useCallback(async () => {
    return handleApiCall(() => apiRef.current.post('/gmail/trash/empty'));
  }, [handleApiCall]);

  const addLabel = useCallback(async (messageId: string, labelId: string) => {
    return handleApiCall(() => 
      apiRef.current.post(`/gmail/messages/${messageId}/modify`, { 
        addLabelIds: [labelId], 
        removeLabelIds: [] 
      })
    );
  ...
  const getLabels = useCallback(async () => {
    return handleApiCall(async () => {
      const response = await apiRef.current.get('/gmail/labels');
      return response;
    });
  }, [handleApiCall]);
  ...
  const createLabel = useCallback(async (name: string) => {
    return handleApiCall(async () => {
      const response = await apiRef.current.post('/gmail/labels', { name });
      return response;
    });
  }, [handleApiCall]);
  ...
  const updateLabel = useCallback(async (labelId: string, name: string) => {
    return handleApiCall(async () => {
      const response = await apiRef.current.put(`/gmail/labels/${labelId}`, { name });
      return response;
    });
  }, [handleApiCall]);
  ...
  const deleteLabel = useCallback(async (labelId: string) => {
    return handleApiCall(async () => {
      const response = await apiRef.current.delete(`/gmail/labels/${labelId}`);
      return response;
    });
  }, [handleApiCall]);
  ...
  const getAttachment = useCallback(async (messageId: string, attachmentId: string) => {
    return handleApiCall(async () => {
      const response = await apiRef.current.get(`/gmail/messages/${messageId}/attachments/${attachmentId}`, {
  ...
  const sendMessage = useCallback(async (emailData: SendEmailRequest) => {
    return handleApiCall(async () => {
      ...
      if (emailData.attachments && emailData.attachments.length > 0) {
        ...
        const response = await apiRef.current.post('/gmail/messages/send', emailData);
        return response;
      }
    });
  }, [handleApiCall]);
