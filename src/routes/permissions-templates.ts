...
router.get('/templates', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const templates = await prisma.permissionTemplate.findMany({ orderBy: { createdAt: 'desc' } });
  res.json(templates);
}));

router.get('/templates/:id', asyncHandler(async (req: Request, res: Response): Promise<void> => {
...
router.post('/templates', asyncHandler(async (req: Request, res: Response): Promise<void> => {
...
router.put('/templates/:id', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { name, description, permissions } = req.body;
...
router.delete('/templates/:id', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  await prisma.permissionTemplate.delete({ where: { id: req.params.id } });
  res.json({ success: true });
}));

// Appliquer un template à un rôle
router.post('/templates/:id/apply', asyncHandler(async (req: Request, res: Response): Promise<void> => {
...
router.get('/audit', asyncHandler(async (req: Request, res: Response): Promise<void> => {
