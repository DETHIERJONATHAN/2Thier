...
/** Save a base64 photo to storage (local in dev, GCS in prod), return URL */
async function savePointagePhoto(base64Data: string, prefix: string): Promise<string> {
  const cleanBase64 = base64Data.replace(/^data:image\/\w+;base64,/, '');
  const buffer = Buffer.from(cleanBase64, 'base64');
  const filename = `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.jpg`;
  const key = `pointages/${filename}`;
