...
const TelnyxPage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      message.success('Données Telnyx chargées avec succès');
    } catch (error) {
  ...
  const handleEndCall = async () => {
    ...
    try {
      await stableApi.post(`/telnyx/calls/${currentCall.id}/hangup`);
  ...
  const handleToggleMute = async () => {
    ...
    try {
      ...
      await stableApi.post(`/telnyx/calls/${currentCall.id}/${action}`);
      setIsMuted(!isMuted);
      message.success(isMuted ? 'Micro activé' : 'Micro coupé');
    } catch (error) {
      console.error('❌ [Telnyx] Erreur mute/unmute:', error);
      message.error('Erreur lors du changement de micro');
    }
  };
  ...
  const syncWithTelnyx = useCallback(async () => {
    ...
    try {
      ...
      message.error('Erreur lors de la synchronisation Telnyx');
    } finally {
      setSyncing(false);
    }
  }, [stableApi, loadData]);
