...
export const GoogleGroupsPage: React.FC = () => {
  ...
  const fetchGroups = useCallback(async () => {
    ...
    try {
      ...
      message.error('Erreur lors de la récupération des groupes Google.');
        ...
        <div className="flex gap-2 justify-center">
          <Tooltip title="Voir les détails">
            <Button 
              ...
              icon={<EyeOutlined />} 
              size="small"
              onClick={() => message.info(`Affichage du groupe ${record.name}`)}
            />
          </Tooltip>
          <Tooltip title="Gérer les membres">
            <Button 
              ...
              icon={<TeamOutlined />} 
              size="small"
              onClick={() => message.info(`Gestion des membres du groupe ${record.name}`)}
            />
          </Tooltip>
          <Tooltip title="Paramètres">
            <Button 
              ...
              size="small"
              onClick={() => message.info(`Paramètres du groupe ${record.name}`)}
            />
          </Tooltip>
        </div>
      ),
      ...
      <Card>
        <div className="flex justify-between items-center mb-4">
          ...
          <Button 
            type="primary" 
            icon={<PlusOutlined />}
            onClick={() => message.info('Création d\'un nouveau groupe (à implémenter)')}
          >
            Créer un groupe
          </Button>
        </div>
