...
import { Button, Input, message, Typography, Card, Row, Col, Spin, Empty, Tabs, Tag } from 'antd';
import { RocketOutlined, MailOutlined, FileTextOutlined, SearchOutlined, ExperimentOutlined } from '@ant-design/icons';
...
export const GoogleGeminiPage: React.FC = () => {
  ...
  const handleGenerateEmail = async () => {
    if (!emailPrompt.trim()) {
      message.warning('Veuillez saisir un contenu pour l\'e-mail.');
      return;
    }
    setLoading(true);
    try {
      ...
      message.success('E-mail généré avec succès !');
    } catch (error) {
      message.error('Erreur lors de la génération de l\'e-mail.');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleAnalyzeLead = async (leadId: string) => {
    ...
    try {
      ...
      message.success(`Analyse du lead ${leadId} terminée.`);
    } catch (error) {
      message.error('Erreur lors de l\'analyse du lead.');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleGenerateProposal = async () => {
    ...
    try {
      ...
      message.success('Proposition commerciale générée.');
    } catch (error) {
      message.error('Erreur lors de la génération de la proposition.');
      console.error(error);
    } finally {
      setLoading(false);
    }
