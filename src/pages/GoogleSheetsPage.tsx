...
export const GoogleSheetsPage: React.FC = () => {
  ...
  const fetchSheets = React.useCallback(async () => {
    ...
    try {
      ...
      msgApi.error('Erreur lors de la récupération des feuilles de calcul.');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateSheet = async () => {
    if (!newSheetName) {
      msgApi.warning('Veuillez entrer un nom pour la feuille de calcul.');
      return;
    }
    try {
      await api.post('/api/google-drive/files/create', { name: newSheetName, mimeType: 'application/vnd.google-apps.spreadsheet' });
      msgApi.success(`Feuille de calcul "${newSheetName}" créée avec succès !`);
      fetchSheets();
      setIsModalVisible(false);
      setNewSheetName('');
    } catch (error) {
      msgApi.error('Erreur lors de la création de la feuille de calcul.');
      console.error(error);
    }
  };
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          msgApi.success(`Feuille de calcul "${fileName}" supprimée.`);
          fetchSheets();
        } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: GoogleSheet) => (
        <span>
