...
const { Search } = Input;
  // Utiliser l'API items de Collapse (AntD v5)
...
const GoogleMailPageClean: React.FC = () => {
  ...
  const handleSearch = useCallback(async (value: string) => {
    ...
    try {
      ...
      msgApi.error('Erreur lors de la recherche');
    } finally {
      setIsLoading(false);
    }
  }, [msgApi]);
  ...
  const handleToggleStar = useCallback(async (messageId: string, isStarred: boolean) => {
    ...
    try {
      ...
      msgApi.success(isStarred ? 'Message retiré des favoris' : 'Message ajouté aux favoris');
    } catch (error) {
      console.error('[Gmail] ❌ Erreur toggle star:', error);
      msgApi.error('Erreur lors de la modification');
    }
  }, [currentLabelId, loadMailbox, msgApi]);
      ...
      >
        <Form onFinish={handleSendMessage} layout="vertical">
          ...
          <Collapse
            ...
            items={[{
              key: 'advanced',
              label: 'CC, CCI et pièces jointes',
              children: (
                <>
