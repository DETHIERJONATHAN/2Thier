                  ...
                  {section.fields.map(field => {
                    ...
                    return (
                      <div key={field.id} className={`flex flex-col ${colSpan}`}>
                        ...
                        {field.type === 'text' && (
                          <input className="border rounded px-2 py-1" placeholder={field.label} value={values[String(field.id)] || ''} onChange={e => handleChange(String(field.id), e.target.value)} />
                        )}
                        {field.type === 'number' && (
                          <input type="number" className="border rounded px-2 py-1" placeholder={field.label} value={values[String(field.id)] || ''} onChange={e => handleChange(String(field.id), e.target.value)} />
                        )}
                        {field.type === 'date' && (
                          <input type="date" className="border rounded px-2 py-1" value={values[String(field.id)] || ''} onChange={e => handleChange(String(field.id), e.target.value)} />
                        )}
                        {field.type === 'select' && (
                          <select className="border rounded px-2 py-1" value={values[String(field.id)] || ''} onChange={e => handleChange(String(field.id), e.target.value)}>
                        ...
                        {field.type === 'checkboxes' && getOptions(field).length > 0 && (
                          <div className="flex flex-col gap-1">
                            {getOptions(field).map((opt: any, idx: number) => (
                              <label key={opt.id || idx} className="inline-flex items-center gap-2">
                                <input
                                  type="checkbox"
