...
const GoogleMailPage: React.FC = () => {
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      message.error('Erreur lors du chargement des données Gmail');
    } finally {
      setIsLoading(false);
    }
  }, [gmailService]);
  ...
  }, [gmailService, currentLabel, searchQuery]);
  ...
  const markAsRead = async (messageIds: string[], read: boolean = true) => {
    try {
      ...
      await loadMessages();
      message.success(`Messages marqués comme ${read ? 'lus' : 'non lus'}`);
    } catch {
  ...
  const toggleStar = async (messageIds: string[], starred: boolean) => {
    try {
      ...
      await loadMessages();
      message.success(starred ? 'Étoile ajoutée' : 'Étoile retirée');
    } catch {
      message.error('Erreur lors de la modification');
    }
  };
  ...
  const deleteMessages = async (messageIds: string[]) => {
    try {
      ...
      await loadMessages();
      setSelectedMessages([]);
      message.success('Messages supprimés');
    } catch {
