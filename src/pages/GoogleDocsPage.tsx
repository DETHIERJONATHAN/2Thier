...
import { Button, Table, Input, Modal, message, Typography, Card, Row, Tooltip, Spin, Empty } from 'antd';
...
export const GoogleDocsPage: React.FC = () => {
  ...
  const fetchDocs = React.useCallback(async () => {
    ...
    try {
      ...
      msgApi.error('Erreur lors de la récupération des documents Google Docs.');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateDoc = async () => {
    if (!newDocName) {
      msgApi.warning('Veuillez entrer un nom pour le document.');
      return;
    }
    try {
      await api.post('/api/google-drive/files/create', { name: newDocName, mimeType: 'application/vnd.google-apps.document' });
      msgApi.success(`Document "${newDocName}" créé avec succès !`);
      fetchDocs();
      setIsModalVisible(false);
      setNewDocName('');
    } catch (error) {
      msgApi.error('Erreur lors de la création du document.');
      console.error(error);
    }
  };
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          msgApi.success(`Document "${fileName}" supprimé.`);
          fetchDocs();
        } catch (error) {
          msgApi.error('Erreur lors de la suppression du document.');
          console.error(error);
        }
      },
    });
  ...
  const columns = [
    ...
    },
    {
      title: 'Actions',
      key: 'actions',
      render: (_: unknown, record: GoogleDoc) => (
        <span>
