...
async function main() {
  try {
    const fields = await prisma.formField.findMany({
      where: {
        OR: [
          { label: { contains: 'compteur intelligent' } },
          { label: { contains: 'terre aux normes' } },
          { label: { contains: 'présence du couteau de terre' } },
          { label: { contains: 'compteur' } },
          { label: { contains: 'terre' } },
          { label: { contains: 'couteau' } }
        ]
      },
      include: {
        options: true
    ...
    const prixKwhField = await prisma.formField.findFirst({
      where: { label: { contains: 'Prix Kw/h' } }
    });
